...
{
    static int p[],at[],bt[],btx[],wt[],tat[],completed[],n,total,priority[];
	public static void main(String[] args)
	{
            ...
            for(i=0;i<n;i++) {
            	System.out.println("Enter the Arrival Time, Burst Time and Priority of process P"+(i+1)+":");
            	p[i]=i+1;
            	at[i]=sc.nextInt();
            	bt[i]=sc.nextInt();
            ...
            for(i=0;i<total;i++) {
            	...
            	int small=9999;
            	for(j=0;j<n;j++) {
                    if(at[j]<=i && completed[j]==0 && small>priority[j] && bt[j]!=0) {
                    	small=priority[j];
                    	nextjob=j;
                    }
            	}  
